...
<div class="right">
	...
	<div class="search">
		<form method="post"
			action="${pageContext.request.contextPath }/provider/providerlist.html">
			<input name="method" value="query" type="hidden"> <span>供应商编码：</span>
			<input name="queryProCode" type="text" value="${queryProCode }">

			<span>供应商名称：</span> <input name="queryProName" type="text"
				value="${queryProName }"> <input type="hidden"
				name="pageIndex" value="1" /> <input value="查 询" type="submit"
				id="searchbutton"> <a
				href="${pageContext.request.contextPath }/provider/provideradd.html">添加供应商</a>
		</form>
	</div>
	<!--供应商操作表格-->
		...
		<c:forEach var="provider" items="${providerList }" varStatus="status">
			<tr>
				...
				<td><span><a class="viewProvider" href="javascript:;"
						providerid=${provider.id } providername=${provider.proName }><img
							src="${pageContext.request.contextPath }/statics/images/read.png"
							alt="查看" title="查看" /></a></span> <span><a class="modifyProvider"
						href="javascript:;" providerid=${provider.id }
						providername=${provider.proName }><img
